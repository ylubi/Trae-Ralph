...
### 2.3 执行中 (关键)
> **严格规则**: 如果未能更新任务/测试文件，将导致状态丢失。你**必须**执行这些文件更新操作。

- **原子更新 (Atomic Updates)**: 每完成一个子任务，**立即**使用工具更新 `04-ralph-tasks.md` 中的复选框。
  - ❌ 错误做法：在心里默认它完成了，或者只在 XML 状态里报告，或者等到最后才批量更新。
  - ✅ 正确做法：**Code -> Verify -> Update Task File**. 调用 `SearchReplace` 将 `- [ ] 编写 User 模型` 改为 `- [x] 编写 User 模型`。
- **测试验收 (Test Verification)**: 任务完成状态应由验证结果驱动。
  - **验证覆盖 (Coverage)**: 并非所有任务都需要一对一的测试项。
    - **功能开发**: 必须有对应的测试（单元/组件/手动）在 `05-test-plan.md` 中并通过。
    - **集成/流程**: 可能多个任务对应同一个 E2E 测试场景。此时，子任务完成后可标记 `[x]`，但需确保最终 E2E 测试包含在计划中。
    - **配置/基建**: 可通过运行脚本或简易验证确认，无需强制在 `05-test-plan.md` 列出，但建议在任务后备注 `(已验证)`。
  - **自动执行与更新 (Auto-Execute & Update)**:
    - **执行**: 你必须调用工具（如 `RunCommand`）实际运行测试命令或验证脚本。**严禁**“假装运行”或仅凭代码推断结果。
    - **非交互模式 (Non-Interactive)**: 执行测试命令时，必须使用 CI/非交互模式，避免进入 Watch 模式导致挂起。
      - 例如: 使用 `npm test -- --watch=false`、`CI=true npm test` 或 `vitest run`。
    - **更新**: 根据工具返回的实际执行结果，**自动**修改 `05-test-plan.md`。
      - **通过**: 将对应的 `[ ]` 改为 `[x]`。
      - **失败**: 保持 `[ ]`，并在该行下方添加引用块 `> 🔴 失败原因: ...` 记录报错信息，然后尝试修复。
  - **缺失补充**: 如果是核心业务逻辑且未被现有测试覆盖，**必须**补充测试项。
