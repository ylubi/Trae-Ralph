...
2.  **上下文对齐 (Context Alignment)**: 
    - 如果你已经生成了内部任务列表（System Todos），你必须在加载规则后的第一步，**立即**读取 `RALPH_STATE.md`。
    - **强制同步**: 必须将 `RALPH_STATE.md` 中定义的上下文（包括任务文件、测试文件、规范版本）作为唯一真理来源（Source of Truth）。如果你的内部认知与 `RALPH_STATE.md` 不一致，**必须**废弃你的内部状态，并根据 `RALPH_STATE.md` 重建。
